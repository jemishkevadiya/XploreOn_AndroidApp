...
import { View, Text, StyleSheet, FlatList, ScrollView, ImageBackground, TouchableOpacity } from 'react-native';
...
const ItineraryDetailsScreen = ({ navigation }) => {
    ...
    const renderItinerary = ({ item }) => (
        <View>
            ...
            {item.places.map(renderPlace)}

            <Text style={styles.subHeading}>Restaurants Nearby:</Text>
            {item.restaurants.map(renderRestaurant)}
        </View>
    );
...
const styles = StyleSheet.create({
    ...
    backgroundImage: {
        opacity: 0.7, // Makes the background lighter
    },
    gradientOverlay: {
